...
client.auth('blahblahblah', function (err) {
    if (err) {
      console.log(err)
    };
});
...
rl.on('line', function(line) {
  ...
  j=line.split('\t')
	var t = {
    'artid':cryptr.encrypt(j[0]),
    'fname':cryptr.encrypt(j[1]),
    'artimg':cryptr.encrypt(j[2]),
    'artdes':cryptr.encrypt(j[3]),
    'art3d':cryptr.encrypt(j[4]),
    'artvid':cryptr.encrypt(j[5]),
    'artimgs':cryptr.encrypt(j[6])
  }
...
client.hmset('ages2', 'toby', j[0], 'jordan', '40', 'alonso', '24');
